        ...
        >
          <div className="p-4">
            ...
            <ul className="flex flex-wrap w-full p-1 mb-2 text-sm font-medium text-center rounded-md filter-btns text-slate-500 bg-slate-100 nav-tabs dark:bg-zink-500 dark:text-zink-200">
              ...
              <li
                className={`group grow ${filter === 'follower' ? 'active' : ''}`}
          ...
          <SimpleBar className="max-h-[350px]">
            <div className="flex flex-col gap-1">
              {(notification || [])
                ...
                ?.map((item: Notification, index: number) => (
                  <a
                    key={index}
                    href="#!"
                  ...
                  >
                    ...
                    <div className="grow">
                      <h6 className="mb-1 font-medium">
                        {item.boldName && <b>{item.boldName}</b>} {item.name}{' '}
                        {item.price && (
                          <span className="text-red-500">{item.price}</span>
                        )}
                      </h6>
                      <p
                        className={`text-sm text-slate-500 dark:text-zink-300 ${index === notification.length - 3 ? 'mb-3' : 'mb-0'}`}
                      >
